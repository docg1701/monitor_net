...
- [x] Refactor `global.scss` for Theme Support (AC: 2)
    - [x] Remove `@import "@ionic/angular/css/palettes/dark.always.css";`.
    - [x] Import `@ionic/angular/css/palettes/dark.class.css` instead to allow class-based switching.
    - [x] Define default (Light) and Dark variables in `src/theme/variables.scss` or `global.scss` following Ionic standards.
    - [x] [Reference: netmonitor/src/global.scss]
- [x] Refactor Chart Colors in `HomePage` (AC: 1, 3)
    - [x] Identify hardcoded RGBA values in `lineChartData` (currently `rgba(148,159,177,1)` and `0.2` opacity).
    - [x] Replace them with a mechanism to read from CSS variables or use a CanvasGradient/Pattern if needed, OR simply ensure they match the theme variables.
    - [x] Note: Chart.js canvas colors are often JS-side. A common pattern is to read the computed style of a dummy element or the body to get the variable value.
    - [x] [Reference: netmonitor/src/app/home/home.page.ts]
- [x] Verify Dark Mode Persistence (AC: 3)
    - [x] Ensure that adding `class="ion-palette-dark"` (or equivalent) to the body makes the app look like the current "Always Dark" version.
    - [x] [Reference: netmonitor/src/global.scss]
